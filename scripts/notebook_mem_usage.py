...
script_fp=/data/hayley-old/ModelSpace/nbs 
cd $script_fp
conda activate test
python notebook_mem_usage.py --password song >> nbs.log
"""
import argparse
import re
import subprocess
...
def get_running_notebooks():
    ...
    for n in subprocess.Popen(
        ["/nas/home/haejinso/miniconda3/envs/jlab3/bin/jupyter", "lab", "list"], stdout=subprocess.PIPE
    ).stdout.readlines()[1:]:
...
def get_session_info(password=None):
    ...
    # del df_nb["token"] 
    # see:://gist.github.com/aiguofer/5b0a5532de84402e4ced0548f4650f2d?permalink_comment_id=4162343#gistcomment-4162343 
