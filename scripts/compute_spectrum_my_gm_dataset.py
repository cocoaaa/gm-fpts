...
exp_id = now2str()
out_root = Path(f'../outs/my_gm_dataset/{exp_id}')
print('Exp_id: ', exp_id)
...
def main(args):
    ...
    for model_name, img_dirpath in dict_img_dirpaths.items():
        # print(model_name, Path(sample_dir).exists())
        img_dirname = model_name
...
# run
# cd scripts
# nohup python compute_spectrum_my_gm_dataset.py -ks 3 -n 2000 >> log_mygm.txt &
